...
def single_channel_processing(trace, window_size, center, ncheck, r):
    ...
    expired_time = starttime - int(24*3600) # expired in a day 
    ...
    if len(list_taken_time):
        ...
        if concatenated_flag.shape[0]-npts >= window_size:
            ...
            concatenated_flag[-len(segment)-center+1:-center+1] += np.ones(len(segment))
            i_ = 0
            reshaped_flag = []
...
def multi_channel_processing(trace, station_channels, priority_channel, window_size, center, ncheck, r):
    starttime = UTCDateTime(trace.stats.starttime)
    next_starttime = starttime + (trace.stats.delta*(trace.stats.npts))
    redis_key = f"ai_pick_{trace.stats.network}.{trace.stats.station}.{trace.stats.location}.{trace.stats.channel}" 
    redis_value = {"time": [str(starttime), str(next_starttime)],
    ...
    if len(list_taken_time):
        ...
        if min(trim_npts, waveform[channel].shape[0], waveform_flag[channel].shape[0]) >= window_size:
            ...
            for ch in sorted(station_channels):
                waveform_flag[ch][-len(segment)-center+1:-center+1] += np.ones(len(segment))
...
def associate_preparation(data, min_station, buffer_size, r, db):
...
def packing_to_kafka(outputs, producer, topik):
    kafka_data = {}
    if topik.lower()=="pick": 
        # Produce pick
        kafka_data = {
            'network': outputs[0]['station'].split('.')[0],
            'station': outputs[0]['station'].split('.')[1],
            'picks': [
                {
                    'id': output['station'],
    ...
    elif topik.lower()=="cluster": 
        # Produce cluster
        kafka_data = [
            {
                'id': str(output['id']), 
                'origin_time': str(output['origin_time']),
    ...
    elif topik.lower()=="event": 
        # Produce event
        kafka_data = {
            
        }
    elif topik.lower()=="arrival_pick": 
        # Produce arrival_pick
        kafka_data = {
            
        }

    producer.send(topik, json.dumps(kafka_data).encode())
