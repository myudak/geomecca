        ...
        if all(key in kafka_data for key in required_keys):
            ...
            with FileLock(filename1 + ".lock"):
                if os.path.exists(filename1):
                    ...
                    # Write the new trace to a temporary MiniSEED file
                    new_stream = obspy.Stream(traces=trace)
                    new_stream.write(memory_buffer, format='MSEED')

                    # Function to append the new MiniSEED file to the main MiniSEED file
                    def append_mseed_files(main_file, memory_buffer):
                        with open(main_file, 'ab') as mf:
                            memory_buffer.seek(0)
                            mf.write(memory_buffer.read())
                        memory_buffer.close()

                    append_mseed_files(filename1, memory_buffer)
                else:
...
def process(msg):
    try:
        thread = threading.Thread(target=save_archive, args=(msg,))
        thread.start()
        thread.join()  # Ensure the thread is completed
    except Exception as e:
