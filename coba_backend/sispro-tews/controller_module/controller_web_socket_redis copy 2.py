...
async def connect(sid: str, environ: Dict[str, Any]):
    ...
    clients[sid] = True  # Track connected clients

@sio.event
async def disconnect(sid: str):
    logger.info(f"Disconnected: {sid}")
    if sid in clients:
        del clients[sid]  # Remove disconnected clients
    if sid in tasks:
        tasks[sid].cancel()  # Cancel the associated task
        del tasks[sid]
...
async def waveform(sid: str, data: str):
    ...
    await send_initial_data(sid, channel)  # Send initial data to the connecting client
    task = asyncio.create_task(subscribe_to_channel(sid, channel))  # Create a subscription task
...
async def fetch_valid_data(channel: str) -> list:
    ...
    messages = await redis_client.lrange(f'{channel}_history', 0, -1)  # Limit to the last 100 messages
...
async def subscribe_to_channel(sid: str, channel: str):
    ...
    try:
        while clients.get(sid, False):  # Only continue if the client is still connected
            ...
            if message and message['type'] == 'message':
                try:
                    ...
                    sorted_valid_data = sorted(valid_datas, key=lambda x: parse_datetime(x['starttime']))
                    try:
                        await sio.emit(f'data-{channel}', sorted_valid_data, room=channel)  # Emit to all clients
            ...
            await asyncio.sleep(0.1)  # Short delay to avoid overloading
