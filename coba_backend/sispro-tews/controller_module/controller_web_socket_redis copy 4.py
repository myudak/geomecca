...
async def waveform(sid: str, data: str):
    ...
    try:
        ...
        task = asyncio.create_task(send_initial_data(sid, channel))
        tasks[sid] = task
    except Exception as e:
...
async def fetch_valid_data(channel: str, batch_size: int = 100):
    ...
    # while True:
    messages = await redis_client.lrange(f'{channel}_history', 0, 4000)
        # if not messages:
        #     break  # Exit the loop if no messages are returned
        # start += batch_size
    # valid_datas = json.loads('[' + ','.join(messages) + ']') 
    valid_datas = [json.loads(message) for message in messages]
