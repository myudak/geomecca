...
async def fetch_valid_data(channel: str) -> list:
    ...
    messages = await redis_client.lrange(f'{channel}_history', 0, 6000)  # Limit to the last 100 messages
    ...
    if channel == "IA.SNSI..SHZ":
        print(df)

    def fill_with_mean(waveform):
        arr = np.array(waveform, dtype=float)
        mean = np.nanmean(arr)           # mean of only the non-NaN values
    ...
    valid_df['waveform'] = valid_df['waveform'].apply(lambda w: list(fill_with_mean(w)))
    if channel == "IA.SNSI..SHZ":
        print(valid_df)
