...
from services.arrival_service import arrival_get_all_service
from services.arrival_service import arrival_get_detail_service
from services.arrival_service import arrival_update_service
from services.arrival_service import arrival_get_by_station_service
...
router = APIRouter(prefix="/api/v1/arrival")
...
async def arrival_get_by_detail(
    ...
    arrival_id = req_info["arrival_id"]

    response = arrival_get_detail_service(db, arrival_id, current_user)

    return response


@router.get("/getbystation")
async def arrival_get_by_station(request: Request):
    req_info = request.query_params
    ...
    response = arrival_get_by_station_service(db, station_id, start_date, end_date)

    return response


@router.get("/getall")
async def arrival_get_all(
    current_user: dict = Depends(get_current_user),
    # db: Session = Depends(get_database_session),
):

    response = arrival_get_all_service(db, current_user)
...
async def arrival_update(
    ...
    response = arrival_update_service(
        db,
        arrival_id,
        timestamp,
    )
