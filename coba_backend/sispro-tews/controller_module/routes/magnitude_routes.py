...
from services.magnitude_service import magnitude_get_all_service
from services.magnitude_service import magnitude_get_detail_service
...
async def magnitude_get_by_detail(
        request: Request,
    ...
    magnitude_id = req_info["magnitude_id"]

    response = magnitude_get_detail_service(
        db,
        magnitude_id,
        current_user)
...
@router.get("/getall")
async def magnitude_get_all(
        current_user: dict = Depends(get_current_user)
        # db: Session = Depends(get_database_session),
    ):

    response = magnitude_get_all_service(
        db,
        current_user)
