...
app.include_router(routes.fdsn_route.router)
...
async def add_process_time_header(request: Request, call_next):
    ...
    log += "\n"+ str(datetime.datetime.now()) +" Response "+trace_id+" : "+ str(response_body)
    log += "\n"+ str(datetime.datetime.now()) +" Response Time "+trace_id+" : "+ str(int(process_time*1000))+"\n"
    # log += "\nTimestamp "+trace_id+" : "+ str(datetime.datetime.now())+"\n"
    logging.basicConfig(filename="./logs/logs_fdsn_module_"+str(today_date)+".log", level=logging.INFO)
    # logging.info(log)
...
async def hello_world():
    return {"message": "Hello, this is fdsn module backend!"}
