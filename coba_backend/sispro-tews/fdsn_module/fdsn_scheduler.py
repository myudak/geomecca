...
from repositories.job_repository import job_find_by_name_repository
import sys
...
def fetch_data(server_fdsn, network, code, location, channel):
    print(server_fdsn, network, code, location, channel)
    get_waveform_data(db, server_fdsn, network, code, location, channel)

# Define a function for processing each row
def process_row(row):
    for channel in row["channel"]:
        fetch_data(row["server_fdsn"], row["network"], row["code"], row["location"], channel)
...
async def profile_process():
    ...
    try:
        job_data = job_find_by_name_repository(db, "fdsn")
        if job_data.get("status") == 0:
            if regional != "":
                db_users = db["user"]
                ...
                for station_id in station_ids:
                    ...
                    station_data["server_fdsn"] = [station_data["server_fdsn"]] + server_fdsn
