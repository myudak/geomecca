...
class MySeedLinkClient(EasySeedLinkClient):
    ...
    def save_stream_to_mseed(self, trace):
        try:
            ...
            waveform2 = np.require(trace.data, dtype=np.float64)
            import time
            ...
            channel = trace.stats.network+"."+trace.stats.station+".."+trace.stats.channel
...
def run_clients(arr_datas):
    """
    Uses threading to handle I/O-bound tasks concurrently within each process spawned by multiprocessing.
    """

        # results.append(result)
    with ProcessPoolExecutor(max_workers=len(arr_datas)) as executor:
        future_to_client = {
            executor.submit(run_client, data['seedlink_url'], data['network'], data['station'], data['channel']): data
        ...
        for future in as_completed(future_to_client):
            ...
            try:
                future.result()
                print(f"Completed {data['station']}")
            except Exception as e:
                print(f"Task failed for {data['station']}: {e}")

    # print(f"Batch processed in {time.time() - start_time} seconds")
...
async def profile_process():
    ...
    try:
        ...
        if not os.path.isfile("./data/station.csv"):    
            ...
            #                 future = executor.submit(run_client, seedlink_url, row["network"], row["code"], channel)
            ...
            #             channels = row["channel"]
            #             # for channel in channels:
            #             client_thread = executor.submit(run_client, seedlink_url, row["network"], row["code"], channels)
            ...
            # arr_results = split_array(station_will_runs, total_core)
            
            # with Pool(total_core) as pool:
            #     pool.map(run_clients, arr_results)
            chunk_size = max(1, len(station_will_runs) // total_cores)
            station_chunks = [station_will_runs[i:i + chunk_size] for i in range(0, len(station_will_runs), chunk_size)]

            # from multiprocessing import Pool
            # with Pool(processes=total_cores) as pool:
            #     results = pool.map(run_clients, station_chunks)

            # with ThreadPoolExecutor(max_workers=total_cores) as pool:  # Using a more reasonable number of threads
            #     results = list(pool.map(run_clients, station_chunks))
            list(pool.imap_unordered(run_clients, station_chunks))
