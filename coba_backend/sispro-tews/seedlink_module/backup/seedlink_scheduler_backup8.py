...
def run_client( 
    ...
    while True:
        try:
            ...
            client = MySeedLinkClient(server_url=sdata["seedlink_url"]) 
            for channel in sdata["channel"]:
                client.select_stream(sdata["network"], sdata["station"], channel)
            ...
            print("Warning caught as exception "+sdata["station"]+"  "+str(w))
        except Exception as e:
            print("Client run terminated with exception "+sdata["station"]+"  "+str(e))
        finally:
            # if client != None:
            #     client.close()
            #     del client
...
def run_clients(station_urls):
    """
    Uses threading to handle I/O-bound tasks concurrently within each process spawned by multiprocessing.
    """

    with ThreadPoolExecutor(max_workers=len(station_urls)) as executor:
        executor.map(run_client, station_urls)
    ...
    try:
        ...
        if not os.path.isfile("./data/station.csv"):    
            ...
            with multiprocessing.Pool(processes=total_cores) as pool:
                pool.map(run_clients, station_chunks)
