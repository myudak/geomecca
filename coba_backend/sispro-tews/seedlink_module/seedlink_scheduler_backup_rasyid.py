    ...
    def save_stream_to_mseed(self, trace):
        try:
            ...
            if os.path.isfile(path_save_day+"/"+day_mseed_filename) == False:
                ...
                try:
                    # st1 = read(path_save_day+"/"+day_mseed_filename, format='MSEED', details=True,headonly=True, loose=True)
                    # existing_traces = [
                    ...
                    print(f"Caught an exception 1111: {e}")
                    self.on_seedlink_error()
                except Exception as e:
            ...
            if os.path.exists(filename1):
                ...
                try:
                    ...
                    print(f"Caught an exception 22222: {e}")
                    self.on_seedlink_error()
                except Exception as e:
...
async def profile_process():
    ...
    try:
        ...
        if not os.path.isfile("./data/station.csv"):    
            ...
            # await asyncio.gather(*client_threads)
