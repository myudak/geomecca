...
# Fungsi untuk mengatur dan memulai eksekusi proses
def start_clients(station_will_runs):
    arr_results = split_array(station_will_runs, MAX_CLIENT_PROCESS)
    with ProcessPoolExecutor() as executor:
        futures = [executor.submit(run_client, result_chunk) for result_chunk in arr_results]
...
async def main():
    try:
        if regional != "":
            db_users = db["user"]
            user_data = db_users.find_one({"username": regional})
            station_ids = user_data["stations"]
            db_station = db["station"]

            station_datas = []
            for station_id in station_ids:
                station_data = db_station.find_one({'_id': ObjectId(station_id)})
        ...
        if not os.path.isfile("./data/station.csv"):    
            ...
            for seedlink_url in seedlink_urls:
                ...
                await loop.run_in_executor(None, start_clients, station_will_runs)
