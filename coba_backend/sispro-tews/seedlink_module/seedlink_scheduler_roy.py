...
from configuration.redis import publish_redis_message, publish_redis_message_general, store_redis_message
...
REPLICA_COUNT = int(os.getenv('seedlink_replica_count'))
...
class MySeedLinkClient(EasySeedLinkClient):
    def __init__(self, server_url, autoconnect=True, db_station={}):
        self.threadpool = ThreadPoolExecutor(max_workers=10)
        if db_station != None:
            self.db_station = pd.DataFrame(db_station)
    ...
    def save_stream_to_mseed(self, trace):
        try:
            ...
            station_data = self.db_station[(self.db_station['network'] == trace.stats.network) &
                                           (self.db_station['station'] == trace.stats.station)]
            if not station_data.empty:
                station_data = station_data.iloc[0].to_dict()
            else:
...
async def batch_run_client(arr_datas):
    ...
    while True:
        try:
            client = MySeedLinkClient(server_url=arr_datas[0]['seedlink_url'], db_station=arr_datas) 
    ...
    try:
        ...
        if not os.path.isfile("./data/station.csv"):
            ...
            for seedlink_url in seedlink_urls:
                ...
                # If service is replicated, run only part of all stations according to replica id
                if REPLICA_ID != None:
                    print("Detected as replica number", REPLICA_ID)
                    station_per_replica = math.ceil(len(station_will_runs) / REPLICA_COUNT)
                    start = (int(REPLICA_ID)-1) * station_per_replica
                    end = int(REPLICA_ID) * station_per_replica
                    station_will_runs = station_will_runs[start:end]
                    print(f"Station per replica: {station_per_replica}")
                    print(f"Processing total of {end-start} from index {start} to {end}")
                else:
                    print("No replica, processing all stations")
